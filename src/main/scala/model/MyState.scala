...
class MyState(val toMove: Boolean,
  ...
  private def stateAfterMove(m: PutMove) = {
    val actualP = positions(m.destination.name)
  ...
  private def stateAfterMove(m: ShiftMove) = {
    val actualO = positions(m.origin.name)
    val actualD = positions(m.destination.name)
  ...
  def moveCreatesMill(m: Move, c: Boolean): Boolean = stateAfterMove(m).isPartOfMill(m.destination, c)
